...
num_samples_Red = nrow(red_wine)

# Number of features (columns)
num_features_Red = ncol(red_wine)

# Feature names
feature_names_Red = colnames(red_wine)

# Data types
feature_types_Red = sapply(red_wine, class)
...
# Feature types (data types of each column)
feature_types_red <- sapply(red_wine, class)

# Summary
cat("Number of red_wine samples:", num_samples_red, "\n")
cat("Number of red_wine features:", num_features_red, "\n\n")
cat("Feature names and types:\n")

# Create summary statistics for numeric variables (mean, median, sd, min, max)
numeric_stats_red <- red_wine %>%
  select(where(is.numeric)) %>%
  summary()

# Combine feature information (names, types, and measurement levels)
...
info_df_red <- data.frame(
  Feature_red = feature_names_red,
  Type_red = feature_types_red,
  Measurement_red = measurement_levels_red[feature_names_red]
)
...
print(numeric_stats_red)


# White Wine Summary Statistics

# Number of samples (rows)
num_samples_White = nrow(white_wine)

# Number of features (columns)
num_features_White = ncol(white_wine)

# Feature names
feature_names_White = colnames(white_wine)

# Data types
feature_types_White = sapply(white_wine, class)
...
# Summary
cat("Number of White wine samples:", num_samples_white, "\n")
cat("Number of white wine features:", num_features_white, "\n\n")
cat("Feature names and types:\n")
...
# Create summary statistics for numeric variables (mean, median, sd, min, max)
numeric_stats_white <- white_wine %>%
  select(where(is.numeric)) %>%
  summary()

# Combine feature information (names, types, and measurement levels)
measurement_levels_white <- c(
  "fixed.acidity" = "Ratio",
  "volatile.acidity" = "Ratio",
  "citric.acid" = "Ratio",
  "residual.sugar" = "Ratio",
...
info_df_white <- data.frame(
  Feature_white = feature_names_white,
  Type_white = feature_types_white,
  Measurement_white = measurement_levels_white[feature_names_white]
)
...
print(numeric_stats_white)


# Combined Summary of Both Red and White Wine Data
cat("\nCombined Wine Data Summary:\n")
combined_summary <- list(
  "Red Wine Summary" = info_df_red,
  "White Wine Summary" = info_df_white
)

# If you want to view them together in a summary format
...
kable(feature_info, format = "markdown", align = "l")
```
...
wine_table$widths <- rep(unit(1 / ncol(wine_table), "npc"), ncol(wine_table))

# Title text
title_text <- "Red Wine Quality Dataset (First 10 Rows)"
wrapped_title <- paste(strwrap(title_text, width = 60), collapse = "\n")

# Create title grob centered
title <- textGrob(wrapped_title,
                  gp = gpar(fontsize = 12, fontface = "bold"),
                  just = "center", x = 0.5)

# Add vertical padding by composing a grob tree:
padded_title <- grobTree(
  rectGrob(gp = gpar(col = NA, fill = NA)),  # invisible rect, fills space
  title
)

# Calculate height to add enough vertical space manually
extra_height <- unit(10, "mm")  # adjust as needed

full_table <- gtable_add_rows(wine_table, heights = grobHeight(padded_title) + extra_height, pos = 0)

full_table <- gtable_add_grob(full_table, grobs = padded_title, t = 1, l = 1, r = ncol(full_table))
...
# Title text
title_text <- "White Wine Quality Dataset (First 10 Rows)"
wrapped_title <- paste(strwrap(title_text, width = 60), collapse = "\n")
